...
const isRoomLoaded = computed(() => props.room && props.room.id && props.room.name && props.room.code)

const userCount = computed(() => allSelectedDates.value.length)

const userNames = computed(() => allSelectedDates.value.map(user => user.user_name).join(', '))
...
async function fetchAllSelectedDates() {
  ...
  try {
    ...
    const userDates = allSelectedDates.value.find(user => user.user_name === userStore.name)
    if (userDates) {
      selectedDates.value = userDates.available_days
    }
  } catch (error) {
    console.error('Error fetching dates:', error)
    alert(`Error fetching dates: ${(error as Error).message}. Please try again.`)
  }
}
...
function calculateCommonDates() {
  ...
  commonDates.value = dateArrays.reduce((a, b) => a.filter(date => b.includes(date)))
}
  ...
  <div v-if="isRoomLoaded" class="flex flex-col gap-4 max-w-md mx-auto">
    ...
    <div class="bg-green-400 p-6 rounded shadow-md">
      ...
      <p v-else class="text-white">
        No dates have been entered yet.
      </p>
      <p v-if="userCount === 1" class="text-white mt-2">
        These are the dates selected by the first user. Common dates will be shown when more users enter their availability.
      </p>
    </div>
